...
class RoomService {
    ...
    async reOpenRoom(ownerId, roomInfo) {
        ...
        const data = await rentalContract.reOpenRoomForRent(room, user?.wallet?.walletAddress, basePrice, deposit);
    ...
    async autoCreateInvoice(contract) {
        const updateDemand = await this.checkUpdateServiceDemand(contract?.room._id);
        if (!updateDemand) {
            const date = new Date();
            const oldDemands = await demandService.getListServiceDemandRoomAtMonth(contract?.room._id, date.getMonth());
            const demands = await demandService.getListServiceDemandRoomAtMonth(contract?.room._id, date.getMonth() + 1);
            let datas = [];
