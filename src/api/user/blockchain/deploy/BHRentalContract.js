...
RentalContract.signByRenter = async (
  ...
  const signRenter = await Web3Services.createSigner(renterAddress);
  ...
  // Web3Services.sendTransaction(transactionDetails)
...
RentalContract.setRoomForRent = async (
  ...
  await userWalletService.changeBalance(
    ...
    5000,
    null,
    USER_TRANSACTION_ACTION.CREATE_ROOM
  );
...
RentalContract.payForRentMonth = async (
  ...
  const userBalance = await RentalContract.getUserBalance(signRenter.address);
  if (
    userBalance < valueInvoiceFee + valuePay ||
    wallet.balance < invoiceAmount + rentAmount
  )
